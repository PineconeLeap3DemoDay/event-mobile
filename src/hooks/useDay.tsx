import { getNextDayOfSelectedDay, getFirstDayOfMonth, getLastDayOfMonth } from "../utils";
export function getnextday(date: Date) {
    const date1 = (new Date(date).toISOString().split('-').map((el) => parseInt(el)))
    date1[2]++;
...
export default function useDay() {
    ...
    const lastDayOfThisMonth = getLastDayOfMonth(thisYear, thisMonth + 1).toISOString().slice(0,10);
